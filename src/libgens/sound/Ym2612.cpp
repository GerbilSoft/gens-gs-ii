 ...
 */
void Ym2612::zomgSave(Zomg_Ym2612Save_t *state) const
{
	// Copy the registers.
	memcpy(state->reg, d->state.REG, sizeof(state->reg));

	// TODO: Save other counters and stuff!
}

/**
 * Restore the YM2612 state.
...
int YM2612_Save_Full(gsx_v7_ym2612 *save)
{
	...
	{
		gsx_v7_channel_t *chanGSX = &save->channels[channel];
		...
		{
			gsx_v7_slot_t *slotGSX = &chanGSX->slot[slot];
