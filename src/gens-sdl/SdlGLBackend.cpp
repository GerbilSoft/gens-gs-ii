 ...
 * @param fb MdFb.
 */
void SdlGLBackend::set_video_source(LibGens::MdFb *fb)
{
	if (m_fb == fb)
		return;
 ...
 */
void SdlGLBackend::reallocTexture(void)
{
	if (m_tex > 0) {
		glDeleteTextures(1, &m_tex);
	}
 ...
 * Recalculate the texture rectangle.
 */
void SdlGLBackend::recalcTexRectF(void)
{
	if (!m_fb)
		return;
